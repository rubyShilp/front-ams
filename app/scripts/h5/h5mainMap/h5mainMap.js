...
import {
  ...
  getProvAttendSum,
  getAllProvinces,
  getCityAttendRecords,
  getProvAttendRecords,
  getCityAttendRecordSum,
} from "./h5mainMap.server";
      ...
      for (let i = 0; i < colorArr.length; i++) {
        if (colorArr[i].count <= 0) {
          newArr.push({
            name:
              type === 1
                ? colorArr[i].provname.substr(
                    0,
                    colorArr[i].provname.length - 1
                  )
                : colorArr[i].cityname,
    ...
    showProvince(eName, param) {
      ...
      if (this.prodata) {
        ...
        for (let i = 0; i < this.prodata.length; i++) {
          let name = this.prodata[i].name.substr(
            0,
            this.prodata[i].name.length - 1
          );
          if (param === name) {
