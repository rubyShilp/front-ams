...
//绘制围栏接口
export const fentch = async (params) => {
  let result = await post("/alading/api/fence/createFence", params);
  return result.data;
};
//查询电子围栏接口
export const fentchQuery = async (params) => {
  let result = await get("/alading/api/fence/getFenceList?schoolcode="+params);
  return result.data;
};
//93.	修改电子围栏接口
export const fentchUpdate = async (params) => {
  let result = await post("/alading/api/fence/updateFence", params);
  return result.data;
};
//95.	删除电子围栏接口
export const fentchDel = async (params) => {
  let result = await get("/alading/api/fence/batchRemoveFences?schoolcodes="+params);
  return result.data;
};
//批量删除电子围栏接口
// export const fentchmoreDel = async (params) => {
//   let result = await get(
//     "/alading/api/attend/rule/batchRemoveAttendRule?rulecodes=" + params
//   );
//   return result.data;
